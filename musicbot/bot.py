...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='Greggs Radio ©2018', icon_url='http://www.stickpng.com/assets/images/5859b19e711f64423aa5e051.png')
        e.set_author(name=self.user.name, url='https://youtube.com/GreggsNE', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, contact staff').format(prefix)
        if not self.is_all:
